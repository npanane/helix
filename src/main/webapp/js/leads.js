...
function initializeHotListGrid() {
    ...
    hotListGrid.setHeader("Hot List");
...
function populateHotListGridRequestHandler(data) {
    var lead = data.getElementsByTagName("leads/lead");
    if (lead == null || lead.length < 1) {
        lead = data.getElementsByTagName("lead");
    }
    for (var i = 0; i < lead.length; i++) {
        hotListGrid.addRow(lead[i].getAttribute("leadId"),
...
function getAddressDetails(rowId){
    ...
    $.ajax({
        url: 'lead/loadMailingAddressInfo',
        data: ({leadId:rowId}),
        success: function(data) {
...
function saveMailingAddressInfo() {
    ...
    $.ajax({
        url: 'lead/updateMailingAddressInfo',
